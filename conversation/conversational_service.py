...
class ConversationalRAGService:
    ...
    def process_query(
        ...
        try:
            ...
            # Enable streaming for any active LLM provider (local, openrouter)
            use_streaming = False
            try:
                ...
                # Fallback to checking self.current_provider
                use_streaming = (self.current_provider == 'local')
                if use_streaming:
                    self.logger.info("Streaming enabled with local provider (fallback)")
